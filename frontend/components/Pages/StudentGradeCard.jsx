  ...
  useEffect(() => {
    ...
    const fetchStudentData = async () => {
      try {
        ...
        const resultResponse = await axios.get(`http://localhost:5000/api/result/${id}/student/${studentId}`);
        const result = resultResponse.data.studentResult;
        

        const studentResponse = await axios.get(`http://localhost:5000/api/students/${result.studentID}`);
        const student = studentResponse.data;
        
     
        const formattedData = {
          name: student.studentName,
          class: student.classId.classId, 
